...
const STANDARD_TERMS_MD = `# Standard Terms
...
export function getConfidentialityTermText(data: NdaFormData): string {
  return data.confidentialityTermType === 'years'
    ? `${data.confidentialityTermYears} year(s) from Effective Date, but in the case of trade secrets until the Confidential Information is no longer considered a trade secret under applicable laws`
    : 'in perpetuity';
}
  ...
  return md
    ...
    .map((para) => {
      ...
      if (trimmed.startsWith('# ')) {
        return `<h2 class="text-xl font-bold mb-4 mt-6 uppercase tracking-wide">${processInline(trimmed.slice(2))}</h2>`;
      }
      return `<p class="mb-3 text-justify leading-relaxed">${processInline(trimmed)}</p>`;
    })
    .filter(Boolean)
    .join('\n');
}

export function processStandardTerms(data: NdaFormData): string {
  const values: Record<string, string> = {
    ...
    'Governing Law': data.governingLaw || '[Governing Law not specified]',
    Jurisdiction: data.jurisdiction || '[Jurisdiction not specified]',
  };

  let html = STANDARD_TERMS_MD;
  for (const [key, value] of Object.entries(values)) {
    html = html.replaceAll(
      `<span class="coverpage_link">${key}</span>`,
      `<span class="filled-value">${escapeHtml(value)}</span>`
    );
  }
